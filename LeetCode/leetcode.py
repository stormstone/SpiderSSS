...
path_df_problems = 'df_problems.csv'
...
def parse_links():
    ...
    for a in lst_problems:
        href = a.get_attribute('href')
        title = a.text.strip()
        df_links = df_links.append([[title, href, False]])

    df_links.columns = ['title', 'href', 'is_crawled']
...
def parse_problem(title, href):
    ...
    count_star = driver.find_element_by_xpath('//button[@class="css-1ey88ll-Button e5i1odf1"]//span').text
    ...
    # print(difficult, count_star, count_pass, count_commit)
    df_tmp = pd.DataFrame()
    df_tmp = df_tmp.append([[title, difficult, count_star, count_pass, count_commit, href]])
    df_tmp.columns = ['title', 'difficult', 'count_star', 'count_pass', 'count_commit', 'href']
    return df_tmp
