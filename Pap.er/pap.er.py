...
def crawler_photo(type_id, photo_count):
    ...
    for photo in photo_data:
        ...
        # 创建一个文件夹存放我们下载的图片
        if not exists('./img/' + str(type_id)):
            makedirs('./img/' + str(type_id))
        ...
        # 准备保存到本地的完整路径
        file_full_name = './img/' + str(type_id) + '/' + file_name_only
        if os.path.exists(file_full_name + '.jpg') == False:
            # 开始下载图片
